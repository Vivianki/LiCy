...
%	\end{figure}
	...
	\begin{figure}[h]
		\caption{\label{plot-post-highpass-filter} Saída esperada após remoção do acoplamento DC, aplicando filtro passa-alta RC de primeira ordem, com defasagem de 45$\degree$. A área hachurada representa tensão }
		\begin{subfigure}{.5\textwidth}
			\centering
			\begin{tikzpicture}
			\begin{axis}[
	...
	\end{equation}
	Como é possível observar na saída do circuito após o filtro passa-altas (\autoref{plot-post-highpass-filter}), a tensão de saída passa por 
	
	Com a onda corrigida com componente $V_{DC} = 1V$, 
