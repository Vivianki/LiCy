...
%	\end{figure}
	...
	Conforme a norma estudada, o último nível de codificação dos dados antes do meio físico é o método de Manchester.	A codificação Manchester divide o bit do dado ao meio, de maneira que a primeira metade deve obrigatoriamente assumir um valor diferente da segunda metade. Em outras palavras, caso a entrada a ser codificada seja 0 por um tempo $2t$, a saída deve ser 01, com cada bit com tempo $t$ de transmissão. Analogamente, caso o bit de entrada seja 1, a saída deve ser 10, como se exemplifica na figura abaixo:
	
	*Figura codificação Manchester*
	...
	*Tabela*
	...
	Além disso, a linguagem de VHDL é uma ferramenta interessante para a construção de máquinas de estado finitos. A síntese desse tipo de circuito é essencial para a implantação em hardware de algoritmos que envolvem iteração, o que é mais um forte justificativa para a escolha desta ferramenta neste estudo.
...
%	\end{figure}
	...
	*Tabela*
	...
	Após a codificação dos dados, é necessário realizar a decodificação deles. Para isso, deve-se reverter os processos aplicados pela seção anterior. No entanto, isso não é tão trivial, dado que vários desses módulos devem realizar correção de erros, tornando o algoritmo completamente diferente. As etapas de decodificação dos dados estão esquematizadas abaixo:
	...
	O transistor deve ser deixado na configuração de fonte comum \autoref{fig_common_source}, agindo como chave entre o circuito do LED, VCC e GND. Quando a FPGA gerar o sinal digital 1, o circuito deve atender $V_{in} \geq V_{(i_{D} > Corrente de LED aceso)}$. Da mesma forma, quando o sinal gerado for 0, é necessário projetar $V_{in} < V_{i_{D} \approx 0}$.
	...
	Será então necessária a utilização de um LED de potência. É muito importante que o LED escolhido tenha resposta luminosa de acordo com o chaveamento, de forma que a tensão fornecida seja proporcional a luz emitida. Como o grupo não tem nenhum medidor de intensidade luminosa, será muito difícil de caracterizar um problema dessa natureza - como por exemplo, a possibilidade do LED atrasar em relação a excitações em seus terminais.
	...
	\begin{figure}[htb]
		...
		\centering
		\includegraphics[width=0.4\textwidth, trim={12cm 9.2cm 12cm 9.5cm},clip]{circuits/led_schematics.pdf}
		\legend{Fonte: Autores}
	\end{figure}
	...
	O comportamento de um amplificador de transimpedância é simples: converter corrente em tensão. Ele normalmente é utilizado com componentes em que resposta em corrente é mais linear que a resposta em tensão, no caso o fotodiodo. 
	
	O amplificador apresenta baixa impedância de entrada para isolar o fotodiodo da voltagem de saída. Ao contrário do fototransistor, que tem corrente de saída a $\mu$A, o fotodiodo gera uma corrente na ordem de grandeza de pA. Para converter essa corrente minúscula a uma voltagem útil para o circuito, é necessário realizar a amplificação desse sinal utilizando um resistor de feedback, rotulado como R1 na \autoref{fig_transimpedance_amp_simple}.
	...
	O circuito apresentado tem voltagem de saída com o seguinte comportamento:

	\begin{equation}
	V_{out} = - I_{fotodiodo} \cdot R_{feedback}
	\end{equation}
	
	Como $I_{fotodiodo}$ é da ordem de pA, $R_{feedback}$ deve ser grande (M$\Omega$). Nota-se que a $V_{out}$ é negativo pois o amplificador se encontra na configuração inversora.

	% ---
	\subsubsection{Filtro Passa-Faixas}\label{hard-bandfilter}
	% ---

	Com intuito de reduzir o ruido em circuitos de amplificação com amplificadores operacionais, especifica-se um modelo de circuito passa-faixas na \autoref{fig_opampdif}.
	...
	Após o estágio de recepção propriamente dito utilizando o Amplificador de Transimpedância e o fotodiodo, obtém-se uma onda com componente $V_{DC} = L$. Essa componente é variável e depende de alterações na intensidade de luz, variações na distância do transmissor com receptor, entre outros fatores. A amplitude da onda depende da corrente  $i_{D}$ do fotodiodo e da resistência de feedback $R_{F}$ do circuito amplificador.
	
	\begin{figure}[h]
		\caption{\label{plot-pre-lowpass-filter} Saída esperada do Amplificador de Transimpedância, com amplitude de $i_{D} \cdot R_{F}$, com componente $V_{DC} = L$ variável.}
		\centering
		\begin{tikzpicture}
			\begin{axis}[
				xmin=0, xmax=25,
				...
				xticklabels={,,},
				ytick={0.3, 1, 1.7},	
				yticklabels={$-A_{i_{D} \cdot R_{F}}$,$L$,$A_{i_{D} \cdot R_{F}}$},
				ylabel near ticks,
	...
	\begin{equation}
	\phi = arctan{\frac{1}{2 \cdot \pi \cdot f_{op} \cdot R \cdot C}}
	\end{equation}
	
	\begin{figure}[h]
		\caption{\label{plot-post-highpass-filter} Saída esperada após remoção da componente DC, aplicando filtro passa-alta RC de primeira ordem.}
		\begin{subfigure}{.5\textwidth}
			\centering
				...
				\addplot[smooth, domain=0:8*pi, color=red]{0.7*sin(deg(x/2))};
				\addplot[domain=0:8*pi]{0};
				\end{axis}
				\end{tikzpicture}
			\label{fig:sub1}
		\end{subfigure}%
	...
	\begin{figure}[h]
		\caption{\label{plot-post-bias1v} Saída esperada após adição de componente DC $V_{DC} = 1V$ fixa.}
		\centering
		\begin{tikzpicture}
		\begin{axis}[
			xmin=0, xmax=25,
			...
			xticklabels={,,},
			ytick={0.3, 1, 1.7},	
			yticklabels={$-A_{i_{D} \cdot R_{F}}$,$1$,$A_{i_{D} \cdot R_{F}}$},
			ylabel near ticks,
			%ylabel style={yshift=-0.5cm},
			xlabel shift = 2 pt,
			xlabel style={yshift=-1cm},
	...
	\end{figure}
	\begin{figure}[h]
		\caption{\label{plot-post-comparator} Saída esperada após o comparador}
		\centering
		\begin{tikzpicture}
			\begin{axis}[
			xmin=0, xmax=25,
			...
			{(0,0) (0, 0.7) (2*pi,0.7) (2*pi,0) (4*pi,0) (4*pi,0.7) (6*pi,0.7) (6*pi,0) (8*pi,0)};
			\end{axis}
		\end{tikzpicture}
		\legend{Fonte: Autores}
	\end{figure}
	Para realizar a etapa de comparação, transformando a onda analógica em onda quadrada, é de altíssima importância que a componente DC seja fixa. Uma solução é remover a componente contínua completamente, para adicioná-la apenas na etapa de comparação. Uma das formas de se fazer isso é utilizando um filtro passa altas, esquematizado na \autoref{} a seguir:

	é realizado um estágio de amplificação com comparação, para transformar o sinal analógico  recebido (até então senoidal) em uma onda quadrada. 
	
	O comparador é um circuito de decisão eletrônico que utiliza um amplificador operacional com alto ganho sem realimentação, como na \autoref{fig_comparator_circuit}.
	
	\begin{figure}[htb]
		\caption{\label{fig_comparator_circuit}Circuito comparador simples com amplificador operacional}
		\centering
		\includegraphics[width=0.5\textwidth, trim={7.5cm 5cm 7.5cm 5.5cm},clip]{circuits/comparator_amp.pdf}
		\legend{Fonte: Autores.}
	\end{figure}
	...
	\begin{figure}[htb]
		\caption{\label{fig_comparator_transfer}Curva de resposta de um comparador não-inversor}
		\centering
		\includegraphics[page=1,width=0.5\textwidth, trim={6.7cm 5.5cm 6.7cm 19.25cm},clip]{circuits/lecture_comparator.pdf}
	...
	Com a sensibilidade ajustada de acordo, é possível que o sinal recebido seja muito semelhante (se não exatamente igual) ao enviado. Após esse estágio pode ser necessário realizar a retificação da onda com um outro comparador, um schmitt trigger hex inverter nesse caso.
