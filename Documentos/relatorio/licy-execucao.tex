...
% ---
	...
	Deve-se observar, no entanto, que o tamanho da mensagem a ser guardado é maior que apenas o tamanho máximo do \textit{payload} da PHY I (1023 \textit{bytes}). Isso acontece porque o módulo anterior ao \textit{Interleaver} - o Codificador Reed Solomon - transforma cada 28 \textit{bits} de mensagem em 60 \textit{bits} (Reed Solomon (15,7)). A mensagem com paridade adicionada ao fim tem portanto 2191 \textit{bytes}. Além disso, ainda deve ser considerado o cabeçalho, sua paridade e seu \textit{padding} (4 + 3 + 8 \textit{bytes}). Portanto o Interleaver deve ser, então, capaz de guardar 2206 \textit{bytes}. Caso a mensagem seja maior que o tamanho máximo do \textit{payload}, ela deve ser dividida em outro \textit{payload}, para ser enviada posteriormente.
	...
	\begin{equation}
	Address = depth_{counter} + iteration \cdot depth
	\end{equation}
	...
	\section{Testes}
	 ...
	 \begin{itemize}
	 	\item Velocidade de download: 480 bits/s;
	 	\item Distância de transmissão: 16cm;
	 	\item BER: ele não recebe bits errados, e sim perde pacotes;
	 	\item Perda de pacotes: para mensagens maiores que 10 caracteres, 30\%.
	 \end{itemize}
 
	 A redução no \textit{download} pode ser atribuída ao \textit{delay} inserido para enviar os dados corretamente para a UART, diminuindo de 11.67kbits/s especificado pela norma IEEE 802.15.7 para 4.8kbits/s. Também pode-se atribuir a essa redução ao \textit{overhead} introduzido pelo pipeline de codificação e decodificação.
